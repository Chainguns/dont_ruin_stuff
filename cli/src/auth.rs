use hyper::{Method, Client, Request , Body, body};
...
pub async fn get_access(action: &str) -> bool {
    ...
    .body(Body::from(format!("{{\"client_token\":\"{}\",\"action\":\"{}\"}}", token, action)))
    .unwrap();
    ...
    match json["opt_in"].as_bool() {
        ...
        None => {
            println!("{}", "error while parsing the response from the authenticator".red());
            false
        }
    }
}
